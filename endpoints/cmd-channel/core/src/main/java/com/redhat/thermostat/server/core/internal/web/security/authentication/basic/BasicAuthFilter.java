...
public class BasicAuthFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext)
            throws IOException {
        String authentication = requestContext
                .getHeaderString(HttpHeaders.AUTHORIZATION);
        if (authentication == null) {
            System.out.println("Auth fail: No 'Authorization' header!");
            throw new NotAuthorizedException("Basic realm=\"thermostat\"");
        }

        if (!authentication.startsWith("Basic ")) {
            System.out.println("Auth fail: Unexpected method. Expected Basic!");
            throw new NotAuthorizedException("Basic realm=\"thermostat\"");
        }

        authentication = authentication.substring("Basic ".length());
        String[] values = new String(
                DatatypeConverter.parseBase64Binary(authentication),
        ...
        if (user == null) {
            System.out.println(
                    "Auth fail: username = " + username + " not found.");
            throw new NotAuthorizedException("Basic realm=\"thermostat\"");
        }

        if (!user.getPassword().equals(password)) {
            System.out.println("Auth fail: password for username = " + username
                    + " did not match our records.");
            throw new NotAuthorizedException("Basic realm=\"thermostat\"");
        }

        System.out.println("Authentication for " + username + " passed.");
